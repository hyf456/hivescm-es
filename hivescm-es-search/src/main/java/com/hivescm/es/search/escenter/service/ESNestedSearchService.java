...
import com.hivescm.es.search.escenter.groovy.NestedUpdateGroovyScritpBuilder;
import com.hivescm.es.search.escenter.util.ESUtil;
import com.hivescm.es.search.escenter.condition.QueryConditionBuilder;
import com.hivescm.es.search.escenter.handler.ESQueryResponseHandler;
...
public class ESNestedSearchService {
	 ...
	 * @param esObject 基础查询参数
	 */
	public DataResult<ESResponse> queryOr(final QueryESObject esObject) {
		final SearchRequestBuilder searchRequestBuilder = esClient.prepareSearch()
				.setIndices(esObject.getIndexName())
				.setTypes(esObject.getTypeName())
				.setExplain(explain);
		...
		try {
			...
			queryConditionBuilder.buildeOr(searchRequestBuilder, esObject);
