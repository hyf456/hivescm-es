...
import com.hivescm.es.search.escenter.condition.CollapseConditonBuilder;
import com.hivescm.es.search.escenter.condition.GroupConditionBuilder;
import com.hivescm.es.search.escenter.condition.SerchSourceBuilder;
import com.hivescm.es.search.escenter.condition.QueryConditionBuilder;
import com.hivescm.es.search.escenter.handler.CollapseResponseHandler;
import com.hivescm.es.search.escenter.handler.ESStatisticResponseHandler;
...
public class ESStatisticServiceImpl {
	...
	public DataResult<Map<String, Number>> statisticByConditions(StatisticESObject esObject) {
		...
		// 构建查询条件
		searchRequestBuilder.setQuery(queryConditionBuilder.innerBuilder(esObject.getSearchConditions()));
		...
		try {
			...
			LOGGER.info("elastic statistic ,rep param:{}, response:{}.", searchRequestBuilder,
					searchResponse);
		} catch (Exception ex) {
	...
	public DataResult<Map<String, ESResponse>> collapse(final CollapseQueryObject esObject) {
		...
		if (!StringUtil.emptyString(esObject.getTypeName())) {
		...
		searchSourceBuilder.fetchSource(Boolean.FALSE);
		...
		try {
			...
			searchResponse = searchRequestBuilder.execute().get();
			LOGGER.info("elastic collapse ,rep param:{}, response:{}.", searchRequestBuilder,
					searchResponse);
		} catch (Exception ex) {
